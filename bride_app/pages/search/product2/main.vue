		...
		<view class="tui-search-history" v-if="history.length>0">
			...
			<view class="tui-history-content">
				<block v-for="(item,index) in history" :key="index">
					<tui-tag type="gray" shape="circle" @click="goto('/pages/discount/product/lists/main?name='+item.name+'&type=1',1)">{{item.name}}</tui-tag>
				</block>
			</view>
		</view>
			 ...
			 search(){
				 console.log("15624");
				   let history = uni.getStorageSync('history') ? uni.getStorageSync('history') : [];
					...
					this.goto("/pages/discount/product/lists/main?type=1&name=" + this.key,1);
			 },
			 cleanRepeat(arr, val) {
				var res = false;
				arr.forEach((v, i) => {
