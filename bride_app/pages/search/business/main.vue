	...
	<view class="container">
		<view class="tui-searchbox">
			<view class="tui-search-input">
				...
				<!-- #endif -->
				<input confirm-type="search" placeholder="信息搜索" :focus="true" auto-focus placeholder-class="tui-input-plholder"
		...
		<view class="tui-search-history" v-if="history.length>0">
			...
			<view class="tui-history-content">
				<block v-for="(item,index) in history" :key="index">
					<tui-tag type="gray" shape="circle" @click="goto('/pages/business/lists/index?name='+item.name,1)">{{item.name}}</tui-tag>
				</block>
			</view>
		</view>
		...
		methods: {
			 search(){
				   let history = uni.getStorageSync('history') ? uni.getStorageSync('history') : [];
					...
					this.goto("/pages/business/lists/index?name=" + this.key,1);
			 },
			 cleanRepeat(arr, val) {
				var res = false;
				arr.forEach((v, i) => {
