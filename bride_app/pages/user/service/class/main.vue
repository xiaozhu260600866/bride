		...
		<view v-if="data.show && data.release">
			...
			<view class="select-Fdiag" v-if="selectDiag">
				<view class="Fdiag-box">
					...
					<view class="conBox">
						<view class="item" @click="goto('/pages/user/service/create_edit/index?category='+parent.name +','+v.label,1)" v-for="v in childrenArr" v-if="!id">{{v.label}}</view>
						<view class="item" @click="goto('/pages/user/service/create_edit/index?category='+parent.name +','+v.label+'&id='+id+'&repeat=1',1)" v-for="v in childrenArr" v-if="id">{{v.label}}</view>
					</view>
		...
		methods: {
			openChildren(parent){
				 if(parent.children && parent.children.length){
					 ...
					 return this.goto('/pages/user/service/create_edit/index?category='+parent.name,1)
				 }
			},
			ajax() {
				this.lists = [ ];
