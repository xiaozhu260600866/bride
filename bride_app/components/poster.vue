	...
	<view>
		...
		<div v-if="show">
			...
			<view id="footer-btn" v-if="tempFilePath">
				<button class="f-btn-orange fs16" type="default" @click='savePhoto'>
	...
	export default {
		...
		methods: {
			...
			imageLoad(e) {
				...
				if(this.product.takeMerchant){
					ctx.fillText(this.product.takeMerchant.userInfo.company_name, 120, this.images[0].height+42)
				}else{
				...
				ctx.setFontSize(12)
				ctx.setFillStyle('#999')
				let nameWidth = this.dis.name.length*12;
