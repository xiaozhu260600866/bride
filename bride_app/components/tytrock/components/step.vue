			...
			<view :class="[direction==='column'?'uni-steps__column-container':'uni-steps__row-container']">
				<view :class="[direction==='column'?'uni-steps__column-line-item':'uni-steps__row-line-item']" v-for="(item,index) in options"
				 :key="index">
					...
					<view v-if="index === active" :style="{backgroundColor:activeColor}" style="width:36upx;height:36upx;line-height:36upx;border-radius: 20upx;text-align:center;">
						<view class="tstep" :style="{color:curtxtColor}">{{ index+1 }}</view>
					</view>
					<view :class="[direction==='column'?'uni-steps__column-check':'uni-steps__row-check']" v-else-if="index<active">
						<uni-icons :color="activeColor" type="checkbox-filled" size="22"></uni-icons>
					</view>
