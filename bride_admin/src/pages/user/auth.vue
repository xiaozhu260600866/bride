<template>
	<section>
		...
		<el-table ref="multipleTable" :data="data.lists" border tooltip-effect="dark" style="width: 100%" @selection-change="selsChange" v-loading="data.listLoading">
			...
			<el-table-column align="center" label="封面人物" width="80">
				<template scope="scope">
				...
				<template scope="scope">
					<el-dropdown>
						...
						<el-dropdown-menu slot="dropdown">
							<el-dropdown-item @click.native="updateMember(scope.row,1)" v-if="data.query.buy == 0">升级会员</el-dropdown-item>
							<el-dropdown-item @click.native="updateMember(scope.row,0)" v-if="data.query.buy == 1">退回散客</el-dropdown-item>
							...
							<!-- <el-dropdown-item @click.native="$refs.table.ajax('/admin/shop/order/lists?user_id='+scope.row.id)">购买记录</el-dropdown-item> -->
							  <el-dropdown-item v-if="!scope.row.openid" @click.native="$refs.qrcord.ajax(siteName+'/userQrcode?user_id='+scope.row.id)">绑定二维码</el-dropdown-item>
							<el-dropdown-item @click.native="$refs.coupon.ajax(scope.row,data.coupon)" >赠送优惠券</el-dropdown-item>
							<el-dropdown-item @click.native="$refs.couponPackage.ajax(scope.row,data.couponPackage)" >赠送券包</el-dropdown-item>
							<el-dropdown-item @click.native="$refs.amount.ajax(scope.row)" >充值余额</el-dropdown-item>
							<el-dropdown-item @click.native="$refs.outAmount.ajax(scope.row)" >退还余额</el-dropdown-item>
							...
							<el-dropdown-item @click.native="upgradeMerchant(scope.row,5)" v-if="scope.row.role == 2">升级为商家</el-dropdown-item>
