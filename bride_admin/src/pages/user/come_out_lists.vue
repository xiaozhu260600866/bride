	...
	<section>
		...
		<el-table ref="multipleTable" :data="data.lists" border tooltip-effect="dark" style="width: 100%" @selection-change="selsChange" v-loading="data.listLoading">
			...
			<el-table-column label="提现姓名" width="110">
					<template scope="scope">{{ scope.row.name ? scope.row.name :'会员已被删除' }}</template>
			</el-table-column>
			<el-table-column align="center" prop="getStatus" label="状态" width="100"> </el-table-column>
			<el-table-column prop="blank_name" label="开户行" min-width="120"> </el-table-column>
			<el-table-column prop="blank_cardno" label="银行卡" min-width="160"> </el-table-column>
			<el-table-column prop="amount" label="提现金额" width="110"> </el-table-column>
			<el-table-column prop="status_content" label="原因" min-width="160" v-if="status == 2"> </el-table-column>
...
export default {
	data() {
		return {
			formAction: '/admin/user/come-out-lists?type=1',
