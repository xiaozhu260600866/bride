	...
	<section>
		...
		<el-table ref="multipleTable" :data="data.lists" border tooltip-effect="dark" style="width: 100%" @selection-change="selsChange" v-loading="data.listLoading">
			...
			<el-table-column label="商家名称" min-width="200">
					<template scope="scope">{{ scope.row.getUser ? scope.row.getUser.userInfo.company_name + scope.row.getUser.userInfo.name  :'已被删除'}}</template>
			</el-table-column>
			<el-table-column label="商家呢称" min-width="200">
					<template scope="scope">{{ scope.row.getUser ? scope.row.getUser.nickname   :'已被删除'}}</template>
			</el-table-column>
			<el-table-column label="提现姓名" min-width="200">
					<template scope="scope">{{ scope.row.name }}</template>
			</el-table-column>
			<el-table-column label="微信商户订单号" min-width="200" v-if="status ==1">
			...
			<el-table-column prop="blank_cardno" label="银行卡" min-width="150"> </el-table-column> -->
			...
			<el-table-column label="审核日期" width="200" >
		...
		<my-status ref="status" style="width:100%" formAction="/admin/merchant/change-come-out-info-status" />
	</section>
</template>
...
export default {
	data() {
		return {
			formAction: '/admin/merchant/come-out-lists?type=1',
