       ...
       <el-col :span="24">
		...
		<el-table ref="multipleTable" :data="data.lists" border tooltip-effect="dark" style="width: 100%" @selection-change="selsChange" v-loading="data.listLoading">
			...
			<el-table-column label="所属产品" min-width="150">
				<template scope="scope">{{ scope.row.getOrder ? scope.row.getOrder.product_str: '' }}</template>
			</el-table-column>
		...
		<el-col :span="24" class="toolbar mtb20">
            ...
            <el-button type="danger" @click="auditAll" :disabled="this.sels.length===0 " v-if="data.query.status == 0">批量发放</el-button>
...
export default {
	data() {
		return {
			formAction: '/admin/investor/come-out-lists?type=0',
			data: this.formatData(this),
			siteName: this.getSiteName(),
			tableName: 'investor_incomes',
			status: 1,
			sels: [], //列表选中列,
			searchFields: {
				searchDate: '',
	...
	methods: {
        auditAll(){
           ...
           if(this.sels.length){
        	   this.getConfirm(title, () => {
        		   this.postAjax("/admin/investor/commission-lists-all", { data: this.sels}, msg => {
		...
		exportExcel() {
		   let url = this.urlApendOpenid("/admin/investor/come-out-lists?type=0&excel=1", this);
