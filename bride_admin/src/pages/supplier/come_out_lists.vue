<template>
	<section>
			...
			<el-form :inline="true" :model="searchFields">
				<div class="Tsec_group">
					...
					<el-form-item label="订单号：">
						<el-input v-model="searchFields.order_no" placeholder="请输入订单号"></el-input>
					</el-form-item>
		...
		<my-status ref="status" style="width:100%" formAction="/admin/supplier/change-come-out-info-status" />
	</section>
</template>
...
export default {
	data() {
		return {
			formAction: '/admin/supplier/come-out-lists?type=1',
	...
	methods: {
		exportExcel(){
			let url = this.urlApendOpenid("/admin/supplier/export-comission?type=1&status=1", this);
