...
export default {
	data() {
		return {
			formAction: '/admin/manager/come-out-lists?type=0',
	...
	methods: {
        auditAll(){
           ...
           if(this.sels.length){
        	   this.getConfirm(title, () => {
        		   this.postAjax("/admin/manager/commission-lists-all", { data: this.sels}, msg => {
		...
		exportExcel() {
		   let url = this.urlApendOpenid("/admin/manager/come-out-lists?type=0&excel=1", this);
