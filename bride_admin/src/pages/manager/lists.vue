<template>
    <section>
            ...
            <el-form :inline="true" :model="searchFields">
                ...
                <div class="Tsec_group">
                    <el-form-item label="区域经理：">
                        <el-input v-model="searchFields.company_name" placeholder="请输入区域经理名称"></el-input>
                    </el-form-item>
                    <el-form-item label="等级：">
                        <el-select v-model="searchFields.manager_lev_id" placeholder="请选择">
                            <el-option v-for="item in data.lev" :label="item.label" :value="item.value">
                            </el-option>
                        </el-select>
                    </el-form-item>
                </div>
                ...
                <div class="Tlnav-po">
                    <el-button type="primary" @click="$refs.createEdit.ajax('',data.lev,data.citys)">新增</el-button>
                </div>
            </el-form>
        </div>
        <el-table ref="multipleTable" :data="data.lists" border tooltip-effect="dark" style="width: 100%"
            ...
            <el-table-column label="代理级别" min-width="150">
                <template scope="scope">{{ scope.row.managerLevName}}</template>
            </el-table-column>
                ...
                <template scope="scope">
                    <el-dropdown>
                        ...
                        <el-dropdown-menu slot="dropdown">
                            <el-dropdown-item @click.native="$refs.createEdit.ajax(scope.row,data.lev,data.citys)">编辑</el-dropdown-item>
...
<script type="text/javascript">
    export default {
        data() {
            return {
                formAction: '/admin/manager/lists',
        ...
        methods: {
            ...
            editField(row, field) {
                this.postAjax("/admin/merchant/editField", {
                    id: row.id,
                    field: field,
                    value: row[field]
                }, msg => {
